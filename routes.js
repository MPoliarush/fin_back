...
const parserFunctions = [getRate, getForeign, getCommod, getIndex, getCrypto, getIPO];

if (isMainThread) {
  const numWorkers = Math.min(os.cpus().length, parserFunctions.length);
  const workers = [];

  for (let i = 0; i < numWorkers; i++) {
    const worker = new Worker(__filename, { workerData: { index: i } }); // Pass the workerData object
    workers.push(worker);
  ...
  const index = workerData.index; // Received index from parent thread
  const parseFunction = parserFunctions[index];

  async function scrapeAndParse() {
    try {
      const parsedData = await parseFunction();
      parentPort.postMessage(parsedData); // Send the parsed data back to the parent thread
    } catch (error) {
